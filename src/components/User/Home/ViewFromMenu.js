...
import "../BusinessService/ViewBusinessService.css";
...
const ViewFromMenu = () => {
  ...
  const fetchServiceDetails = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5196/api/Service/${serviceId}`
      );
  ...
  const handleServiceClick = async (serviceId1) => {
    if (!userId) {
      ...
      if (response.data.length === 0) {
        setSelectedServiceId(serviceId1);
        ...
        <div>
          {serviceDetails ? (
            <div className="business-business-details">
              {/* <h2>Service Name: {serviceDetails.name}</h2> */}
              <p>
          ...
          {services && services.length > 0 ? (
            services.map((service) => (
              <div key={service.id} className="service-card">
                ...
                {serviceImages[service.id]?.length > 0 ? (
            ...
            <p>
              Currently, no businesses are offering this service. Please check
              back later.
            </p>
          )}
        </div>
